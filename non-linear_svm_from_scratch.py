...
"""
Implementation of Non-Linear SVM with polynomial kernel.
"""
...
class MySVM_PolynomialKernel:
    ...
    def lagrange_dual(self, alpha, x, t):
        ...
        for i in ind_sv:
            for k in ind_sv:
                result = result + alpha[i] * alpha[k] * t[i] * t[k] * self.polyKernelTransform(x[i, :], x[k, :])
        result = 0.5 * result - sum(alpha)
        return result

    def polyKernelTransform(self, X, Z):
        """
    ...
    def classify_points(self, alpha, t, x, x_test):
        ...
        for j in range(n):
            p = b
            for i in range(m):
                p += alpha[i] * t[i] * self.polyKernelTransform(x[i, :], x_test[j, :])
    ...
    def calculate_b_optimum(self, alpha, t, x):
        """
        b* = average(t - (w* . f(x)))
        b* = average(t - (∑ αi*ti*f(x).f(x)))

        We can apply the kernel trick the calculate f(x).f(x) without explicitly transforming x to f(x)
        ...
        for s in ind_sv:
            w = 0
            for i in range(m):
                w = w + alpha[i] * t[i] * self.polyKernelTransform(x[i, :], x[s, :])
            w0 = w0 + t[s] - w
        # Take the average
        w0 = w0 / len(ind_sv)
        return w0
...
svm2.plotTrueFunction()
svm2.plot_x_train(x, labels)
plt.savefig('training-data-non-linear-svm.png')
plt.clf()
...
plt.savefig('testing-data-non-linear-svm.png')
...
print("Accuracy on test set ",svm2.accuracy(y_test, labels_test))
